...
#include <Rcpp.h>
#include <idas/idas.h>           // CVODES functions and constants
#include <nvector/nvector_serial.h>  // Serial N_Vector
#include <idas/idas_dense.h>     // CVDense
#include <datatypes.h>
#include <string> 
#include <limits> 
#include <array>
...
int idas_to_Cpp_stl(double t, N_Vector y, N_Vector ydot, N_Vector rr, void* inputs) {
  // Cast the void pointer back to the correct data structure
  data_idas_Cpp_stl* data = static_cast<data_idas_Cpp_stl*>(inputs);
...
//' @export
// [[Rcpp::export]]
NumericMatrix idas_Cpp_stl(NumericVector times, NumericVector states_, 
  ...
  data_idas_Cpp_stl data_model{parameters, forcings_data, neq, model, jacobian};
  ...
  flag = IDAInit(ida_mem, idas_to_Cpp_stl, times[0], y, yp);
  if(flag < 0.0) {
    Rcout << "flag = " << flag << '\n';
    if(y == nullptr) {free(y);} else {N_VDestroy_Serial(y);}
    if(ida_mem == nullptr) {free(ida_mem);} else {IDAFree(&ida_mem);} 
...
//' @export
// [[Rcpp::export]]
List idas_calc_res(SEXP model_, NumericVector t, NumericVector states, 
...
//' @export
// [[Rcpp::export]]
NumericMatrix idas_calc_jac(SEXP jacobian_, NumericVector t, NumericVector states, 
